...
class Trainer(object):
    def __init__(self, args):
        ...
        self.criterion_cwd = CriterionCWD(s_channels, t_channels, norm_type='channel',divergence='kl', temperature=args.cwd_temperature).to(self.device)
    ...
    def train(self):
        ...
        for iteration, (images, targets, _) in enumerate(self.train_loader):
            ...
            if self.args.lambda_cam !=0 and self.args.lambda_pam ==0:
                loss_CAM  = self.criterion_sa(s_outputs[-1], t_outputs[-1], feature_transform="gridCAM")
                sa_loss = self.args.lambda_cam * loss_CAM
            if self.args.lambda_cam ==0 and self.args.lambda_pam !=0:
                loss_PAM = self.criterion_sa(s_outputs[-1], t_outputs[-1], feature_transform="gridPAM")
                sa_loss = self.args.lambda_pam * loss_PAM
            if self.args.lambda_cam !=0 and self.args.lambda_pam !=0:
                loss_CAM, loss_PAM = self.criterion_sa(s_outputs[-1], t_outputs[-1], feature_transform="separately_gridCAMPAM") 
            ...
            if iteration % log_per_iters == 0 and save_to_disk:
                logger.info(
                    ...
                    "|| skd_loss: {:.4f} || cwd_fea_loss: {:.4f} || cwd_logit_loss: {:.4f} " \
                        ...
                        "|| psd_loss: {:.4f} || csd_loss: {:.4f} || sa_loss: {:.4f} ||" \
